...
new class extends Component {
    ...
    public function mount($tryoutId)
    {
        ...
        $this->tryout = Tryouts::where('id', $tryoutId)
            ->where('status', 'active')
            ->select(['id', 'nama', 'image', 'waktu', 'harga', 'batch_id', 'url', 'is_need_confirm', 'is_configurable', 'deskripsi', 'end_time'])
            ->with(['batch'])
            ->first();
        if ($this->tryout == null) {
...
<div class="row">
    @if ($tryout->end_time)
        @php
            // Membulatkan ke bawah sisa waktu dalam menit
            $remainingMinutes = floor(Carbon::parse($tryout->end_time)->diffInMinutes(Carbon::now(), false));
        @endphp

        @if ($remainingMinutes < 0 && $remainingMinutes >= -180)
            {{-- 3 jam = 180 menit --}}
            <div class="col-12 mb-20">
                <div class="alert alert-warning d-flex align-items-start" role="alert">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
                        class="bi bi-exclamation-triangle-fill flex-shrink-0 mt-1 me-3" viewBox="0 0 16 16">
                        <path
                            d="M8.982 1.566a1.13 1.13 0 0 0-1.96 0L.165 13.233c-.457.778.091 1.767.98 1.767h13.713c.889 0 1.438-.99.98-1.767L8.982 1.566zM8 5c.535 0 .954.462.9.995l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 5.995A.905.905 0 0 1 8 5zm.002 6a1 1 0 1 1 0 2 1 1 0 0 1 0-2z" />
                    </svg>
                    <div>
                        <h5 class="alert-heading">Perhatian!</h5>
                        <p class="mb-0">Tryout ini akan segera berakhir pada
                            {{ Carbon::parse($tryout->end_time)->locale('id_ID')->isoFormat('dddd, D MMMM Y, H:mm') }}.
                        </p>
                    </div>
                </div>
            </div>
                        ...
                        <li
                            ...
                            <div class="d-flex align-items-center flex-nowrap gap-1">
                                <iconify-icon icon="mdi:calendar-clock"
                                    class="text-primary-600 icon me-1 text-xl"></iconify-icon>
                                <span class="text-xl">Deadline:</span>
                            </div>
                            <span
                                class="text-xl">{{ Carbon::parse($tryout->end_time)->locale('id_ID')->isoFormat('dddd, D MMMM Y, H:mm') }}</span>
                        </li>
                    @endif
                    <li
